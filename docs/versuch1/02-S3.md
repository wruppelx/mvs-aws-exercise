...
!!! warning "Achtung!"
    Achten Sie darauf, das S3-Bucket in der Region `Europe(Frankfurt)` zu erstellen
...
Der neu erstellte Bucket sollte nun in der Auflistung aller Buckets angezeigt werden.

## Unterordner `Versuch1`erstellen
...
## Unterordner `export` erstellen

Erstellen Sie einen weiteren Unterordner mit dem Namen `export` auf derselben Ebene wie `Versuch1`.

Dieser dient später als Übergabeordner an das Fastly CDN.
